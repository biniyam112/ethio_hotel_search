...
module.exports = {
	create_event: (req, res) => {
		...
		} else if (title && description && authToken) {
      ...
      jwt.verify(authToken, 'PLEASE_CHANGE_IT_LATER', (err, decode) => {
        if (err) {
          ...
          if(decode.type == 'HOTEL') {
            let hotelId = decode.hotel;
            Event.create({ title, description, hotel : hotelId })
				.then((event) => {
	...
	get_one_event: (req, res) => {
		...
		if (event_id) {
			...
			res.status(404).json({ error: true, success: false, message: 'undefined event id' });
		}
	},
	update_event: (req, res) => {
		...
		if (req.fileValidationError) {
			...
			Event.findByIdAndUpdate(event_id, { title, description, hotel }, { new: true })
				.then((event) => {
					console.log(event);
					const images = req.files;
					console.log(images);
			...
			Event.findByIdAndRemove(event_id)
				...
				.catch((err) => {
					if (err.name === 'NotFound' || err.kind === 'ObjectId') {
						res.status(404).json({ error: true, success: false, message: `${err.message}` });
					} else {
						res.status(500).json({ error: true, success: false, message: `${err.message}` });
					}
				});
		} else {
			res.status(500).json({ error: true, success: false, message: `undefined event id ` });
		}
	},
};
